...
class SRUDC(nn.Module):
    def __init__(self, kernel_size=5, base_dim=32, depths=[4, 4, 4, 4, 4, 4, 4], conv_layer=ConvLayer,
        ...
        self.layers_cond1 = nn.Sequential(*[
            TransformerBlock(dim=embed_dims[0], num_heads=1, ffn_expansion_factor=2.66, bias=False,
                             LayerNorm_type='WithBias') for i in range(2)])
        self.downs_cond1_2 = PatchEmbed(patch_size=2, in_chans=embed_dims[0],
                                        embed_dim=embed_dims[1])  ## From Level 1 to Level 2
        self.layers_cond2 = nn.Sequential(*[
            TransformerBlock(dim=embed_dims[1], num_heads=2, ffn_expansion_factor=2.66, bias=False,
                             LayerNorm_type='WithBias') for i in range(3)])

        self.downs_cond2_3 = PatchEmbed(patch_size=2, in_chans=embed_dims[1],
                                        embed_dim=embed_dims[2])  ## From Level 2 to Level 3
        self.layers_cond3 = nn.Sequential(*[
            TransformerBlock(dim=embed_dims[2], num_heads=4, ffn_expansion_factor=2.66, bias=False,
...
def SRUDC_f():
    return SRUDC(kernel_size=5, base_dim=24, depths=[16, 16, 16, 32, 16, 16, 16], conv_layer=ConvLayer,
                 norm_layer=nn.BatchNorm2d, gate_act=nn.Sigmoid, fusion_layer=SKFusion)


def SRUDC_l():
    return SRUDC(kernel_size=5, base_dim=16, depths=[16, 16, 16, 32, 16, 16, 16], conv_layer=ConvLayer,
                 norm_layer=nn.BatchNorm2d, gate_act=nn.Sigmoid, fusion_layer=SKFusion)
