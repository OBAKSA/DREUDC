...
class BNUDC(nn.Module):
    def __init__(self, n_feat_high=72, n_feat_low=32, scale_feats=8, num_blk=6, kernel_size=3, bias=True):
        super(BNUDC, self).__init__()
        self.shallow_feat = nn.Sequential(conv(12, n_feat_high, kernel_size, bias=bias),
                                          SmoothDilatedResidualBlock(n_feat_high))
        self.orsnet = ORSNet(n_feat_high, kernel_size, num_blk)
        self.tail = conv(n_feat_high, 24, kernel_size, bias=bias)

        self.shallow_sub = nn.Sequential(conv(3, n_feat_low, kernel_size, bias=bias),
                                         SmoothDilatedResidualBlock(n_feat_low))
